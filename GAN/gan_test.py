...
for epoch in range(num_epoch):  # 进行多个epoch的训练
    for i, (img, _) in enumerate(dataloader):
        ...
        d_loss_real = criterion(real_out.squeeze(), real_label)  # 得到真实图片的loss
        real_scores = real_out  # 得到真实图片的判别值，输出的值越接近1越好
        # 计算假的图片的损失
        z = Variable(torch.randn(num_img, z_dimension)).to(device)  # 随机生成一些噪声
        fake_img = G(z)  # 随机噪声放入生成网络中，生成一张假的图片
        fake_out = D(fake_img)  # 判别器判断假的图片
        d_loss_fake = criterion(fake_out.squeeze(), fake_label)  # 得到假的图片的loss
        ...
        # g_loss = criterion(output.squeeze(), real_label)  # 得到的假的图片与真实的图片的label的loss
        # # bp and optimize
